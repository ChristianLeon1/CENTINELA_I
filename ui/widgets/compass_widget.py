...
# (Asumiendo que tienes un módulo de log)
...
class Compass(QWidget):
    """
    Un widget de brújula analógica personalizado.
    """
    ...
    def __init__(self, parent=None):
        ...
        # --- Tema Integrado ---
        TEXT_PRIMARY = QColor(PALETTE['TEXT']['PRIMARY'])
        TEXT_SECONDARY = QColor(PALETTE['TEXT']['SECONDARY'])
        ACCENT = QColor(PALETTE['ACCENT']['ACTIVE'])
        DANGER = QColor(PALETTE['STATUS']['DANGER'])
        BG_PANEL = QColor(PALETTE['BACKGROUND']['PANEL'])

        self.setNeedleColor(DANGER.red(), DANGER.green(), DANGER.blue())
        self.setNeedleColorOnDrag(DANGER.lighter(120).red(), DANGER.lighter(120).green(), DANGER.lighter(120).blue())
        self.setScaleValueColor(TEXT_PRIMARY.red(), TEXT_PRIMARY.green(), TEXT_PRIMARY.blue())
        self.setDisplayValueColor(ACCENT.red(), ACCENT.green(), ACCENT.blue())
        self.set_CenterPointColor(ACCENT.red(), ACCENT.green(), ACCENT.blue())
        
        self.bigScaleMarker = TEXT_SECONDARY
        self.fineScaleColor = TEXT_SECONDARY.darker(150)
        self.outer_circle_bg = [[1, QColor(Qt.GlobalColor.transparent)]]
        self.set_scale_polygon_colors([[1, QColor(Qt.GlobalColor.transparent)]])
        self.needle_center_bg = [[0, ACCENT.darker(120)], [1, ACCENT]]
        # --- Fin Tema ---
    ...
    def create_scale_marker_values_text(self):
        ...
        for i in range(self.scalaCount + 1):
            ...
            else: text = ""  # <-- Las divisiones intermedias ahora están vacías
